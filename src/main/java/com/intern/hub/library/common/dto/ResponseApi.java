 ...
 * <p>
 * This record encapsulates all API responses with a consistent format containing
 * status information, optional data payload, and request metadata.
 * </p>
 *
 * <p><b>Success Response Structure:</b></p>
 * <pre>{@code
 * {
 *   "status": null,
 *   "data": { ... },
 ...
 * }</pre>
 *
 * <p><b>Error Response Structure (handled by global exception handler):</b></p>
 * <pre>{@code
 * {
 *   "status": {
 ...
 * @param status   the response status containing code and message (null for success)
...
public record ResponseApi<T>(ResponseStatus status, T data, ResponseMetadata metaData) {
   ...
   */
  static <T> ResponseApi<T> of(ResponseStatus status, T data, ResponseMetadata metaData) {
    return new ResponseApi<>(status, data, metaData);
  }

  /**
   * Creates a simple success response with data only.
   * <p>Status and metadata are set to null for minimal response size.</p>
   *
   * @param data the response data
   * @param <T>  the type of the data payload
   * @return a new ResponseApi instance with data only
   */
  static <T> ResponseApi<T> ok(T data) {
   ...
   * <p>Status is set to null, but metadata is included for request tracing.</p>
   *
   * @param data     the response data
   * @param metaData the response metadata
   * @param <T>      the type of the data payload
   ...
   */
  static <T> ResponseApi<T> ok(T data, ResponseMetadata metaData) {
    return new ResponseApi<>(
        null,
        data,
        metaData
